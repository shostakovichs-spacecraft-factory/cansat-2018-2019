...
static int lsm6ds3_lock( const struct lsm6ds3_dev_s * priv);
// unlock device
static int lsm6ds3_unlock( const struct lsm6ds3_dev_s * priv);
...
static int lsm6ds3_chdev_ioctl( struct file *filep, int cmd, unsigned long arg)
{
	...
	struct lsm6ds3_dev_s * priv  = inode->i_private;
    ...
    {
        rc = lsm6ds3_reset_g_hpf(priv);
    }
    break;

    case SNIOC_LSM6DS3_SETRMODE:
    {
        const lsm6ds3_readmode_t readmode = arg & 0xff00;
        const uint8_t readmode_arg = arg & 0x00ff;
        switch (readmode)
        {
        case LSM6DS3_READMODE_RAW:
        case LSM6DS3_READMODE_FIFO_RAW:
        case LSM6DS3_READMODE_SCALED:
            priv->readmode = readmode;
            priv->readmode_arg = readmode_arg;
            rc = OK;
        break;
    ...
    case SNIOC_LSM6DS3_READCONF:
    {
        struct lsm6ds3_conf_s * thearg = (struct lsm6ds3_conf_s *)arg;
        DEBUGASSERT(thearg != NULL);
        *thearg = priv->conf;
    }
    break;

    case SNIOC_LSM6DS3_WRITECONF:
    {
        const struct lsm6ds3_conf_s * thearg = (const struct lsm6ds3_conf_s *)arg;

        // reset device to remove any fifo data and clear configuration
        rc = lsm6ds3_sw_reset(priv);
        if (rc < 0)
            break;
        ...
        // apply conf
        rc = lsm6ds3_g_push_conf(priv, &thearg->g);
        if (rc < 0)
            break;
        priv->conf.g = thearg->g;

        rc = lsm6ds3_xl_push_conf(priv, &thearg->xl);
        if (rc < 0)
            break;
        priv->conf.xl = thearg->xl;

        rc = lsm6ds3_fifo_push_conf(priv, &thearg->fifo);
        if (rc < 0)
            break;
        priv->conf.fifo = thearg->fifo;
    }
    break;

    case SNIOC_LSM6DS3_GETFIFOSTAT:
    {
        struct lsm6ds3_fifo_status_s * thearg = (struct lsm6ds3_fifo_status_s *)arg;
        rc = lsm6ds3_fifo_pull_status(priv, thearg);
    }
    break;
...
static ssize_t lsm6ds3_chdev_read( struct file *filep,  char *buffer, size_t buflen)
{
    ...
    lockrc = lsm6ds3_lock(priv);
    if (lockrc < 0)
        return lockrc;


    switch (priv->readmode)
    {
    case LSM6DS3_READMODE_RAW:
        {
            struct lsm6ds3_raw_data_s * thearg = (struct lsm6ds3_raw_data_s*)buffer;
            if (buflen < sizeof(struct lsm6ds3_raw_data_s))
            {
                rc = -EINVAL;
                break;
            }

            rc = lsm6ds3_gxl_pull(priv, thearg);
            if (rc < 0)
                break;

            rc = sizeof(struct lsm6ds3_raw_data_s);
        }
        break;
    ...
    case LSM6DS3_READMODE_SCALED:
        {
            struct lsm6ds3_raw_data_s raw;
            struct lsm6ds3_scaled_data_s * thearg = (struct lsm6ds3_scaled_data_s*)buffer;

            if (buflen < sizeof(struct lsm6ds3_scaled_data_s))
            {
                rc = -EINVAL;
                break;
            }

            rc = lsm6ds3_gxl_pull(priv, &raw);
            if (rc < 0)
            {
                rc = -EINVAL;
                break;
            ...
            lsm6ds3_scale_g(&priv->conf.g, raw.g, thearg->g, 3);
            lsm6ds3_scale_xl(&priv->conf.xl, raw.xl, thearg->xl, 3);

            rc = sizeof(struct lsm6ds3_scaled_data_s);
        }
        break;

    case LSM6DS3_READMODE_FIFO_RAW:
        {
            uint16_t * thebuffer = (uint16_t*)buffer;
            ...
            size_t readout_count16 = buflen/sizeof(uint16_t);
            rc = lsm6ds3_fifo_pull_data(priv, thebuffer, readout_count16, priv->readmode_arg);
            // rc already stores correct value
        }
        break;
...
static ssize_t lsm6ds3_chdev_write( struct file *filep,  const char *buffer, size_t buflen)
{
    ...
    // its possible to write device conf
    switch (buflen)
    {
    case sizeof(struct lsm6ds3_conf_s):
        rc = lsm6ds3_chdev_ioctl(filep, SNIOC_WRITECONF, (unsigned long)buffer);
        break;
